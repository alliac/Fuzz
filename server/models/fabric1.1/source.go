...
func (a *Chaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if fn == "Self_area" {
		return a.Self_area(stub, args)
	}

	return shim.Error("Recevied unkown function invocation")
}

func (a *Chaincode) Self_area(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
...
func (a *Chaincode) addIngInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// 	return shim.Error("FoodID can not be empty.")
	// }
	// FoodInfos.FoodIngInfo.IngID = args[1]
	// FoodInfos.FoodIngInfo.IngName = args[2]
...
func (a *Chaincode) addLogInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
func (a *Chaincode) getFoodInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments.")
	}
	FoodID := args[0]
	...
	LogInfosJSONasBytes, _ := stub.GetState(FoodID)

	return shim.Success(LogInfosJSONasBytes)
}
...
func (a *Chaincode) getProInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	ProInfosJSONasBytes, _ := stub.GetState(FoodID)
	// 解析字符串为Json
	var s FoodInfo
	json.Unmarshal(ProInfosJSONasBytes, &s)
	// 遍历Json
	jsonsAsBytes, _ := json.Marshal(s.FoodProInfo)

	return shim.Success(jsonsAsBytes)
}

func (a *Chaincode) getIngInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments.")
	}
	FoodID := args[0]
	...
	// 遍历Json
	jsonsAsBytes, _ := json.Marshal(s.FoodIngInfo)

	return shim.Success(jsonsAsBytes)
}

func (a *Chaincode) getLogInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// 遍历Json
	jsonsAsBytes, _ := json.Marshal(s.FoodLogInfo)

	return shim.Success(jsonsAsBytes)
}

func (a *Chaincode) getLogInfo_l(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	//var Loginfo LogInfo
